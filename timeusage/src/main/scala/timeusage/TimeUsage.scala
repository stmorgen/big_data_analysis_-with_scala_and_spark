...
object TimeUsage extends TimeUsageInterface {
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row.fromSeq(List(line.head) ++ line.tail.map(_.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val primaryNeeds = columnNames.filter(name => List("t01", "t03", "t11", "t1801", "t1803").exists(name.startsWith)).map(column)
    val work = columnNames.filter(name => List("t05", "t1805").exists(name.startsWith)).map(column)
    val leisure = columnNames.filter(name => List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18").exists(name.startsWith)).map(column)

    (primaryNeeds, work, leisure.filter(c => !primaryNeeds.contains(c) && !work.contains(c)))
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when('telfs < 3.0 && 'telfs >= 1.0, "working")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def typeRound(d:Double) = (d * 10).round / 10d
    ...
    summed
      ...
      case ((working, sex, age), primaryNeeds, work, other) => TimeUsageRow(working, sex, age,  typeRound(primaryNeeds), typeRound(work), typeRound(other))
